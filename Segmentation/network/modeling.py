...
def _segm_resnet(name, backbone_name, num_classes, output_stride, output_stride_from_trained, pretrained_backbone, concat_mask_last_layer=False, omit_upscale=False, single_layer_classifier=False):
    ...
    if name=='deeplabv3plus':
        ...
        return_layers = {'layer4': 'out'}
        classifier = FCNHead(inplanes, num_classes, classifier_dilation, single_layer_classifier)
    ...
    if name=='deeplabv3plus' or name=='deeplabv3':
        ...
        model = FCN(backbone, classifier, omit_upscale)
    return model
...
def _load_model(arch_type, backbone, num_classes, output_stride, output_stride_from_trained, pretrained_backbone, concat_mask_last_layer = False, omit_upscale=False, single_layer_classifier=False):

    
    if backbone.startswith('resnet'):
        model = _segm_resnet(arch_type, backbone, num_classes, output_stride=output_stride, output_stride_from_trained=output_stride_from_trained, pretrained_backbone=pretrained_backbone, concat_mask_last_layer=concat_mask_last_layer, omit_upscale=omit_upscale, single_layer_classifier=single_layer_classifier)
...
# FCN
def fcn_resnet50(num_classes=21, output_stride=8, output_stride_from_trained=None, pretrained_backbone=True, omit_upscale=False, single_layer_classifier=False):
    ...
    if output_stride_from_trained == None:
        output_stride_from_trained = output_stride
    return _load_model('fcn', 'resnet50', num_classes, output_stride=output_stride, output_stride_from_trained=output_stride_from_trained, pretrained_backbone=pretrained_backbone, omit_upscale=omit_upscale, single_layer_classifier=single_layer_classifier)
