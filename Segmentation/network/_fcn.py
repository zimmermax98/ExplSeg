...
class FCNHead(nn.Sequential):
    ...
    def __init__(self, in_channels, num_classes, dilation=1, single_layer_classifier=False):
        super(FCNHead, self).__init__()

        if single_layer_classifier:
            self.classifier = nn.Sequential(
                nn.Conv2d(in_channels, num_classes, 1, stride=1, padding=0, bias=False),
            )
        else:
