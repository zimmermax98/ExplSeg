...
def eval_prototypes_cub_parts_csv(csvfile, parts_loc_path, parts_name_path, imgs_id_path, epoch, args, log):
    ...
    print("Number of prototypes in parts_presences: ", len(proto_parts_presences.keys()))
    ...
    for proto in proto_parts_presences.keys():
        ...
        if ('7' in proto_parts_presences[proto].keys() or '8' in proto_parts_presences[proto].keys() or '9' in proto_parts_presences[proto].keys()):
            print("unused part in keys! ", proto, proto_parts_presences[proto].keys(), proto_parts_presences[proto])
            raise ValueError()
    ...
    print("Number of part-related prototypes (purity>0.5): ", prototypes_part_related)

    print("Mean purity of prototypes (corresponding to purest part): ", np.mean(list(max_presence_purity.values())), "std: ", np.std(list(max_presence_purity.values())))
    print("Prototypes with highest-purity part (no contraints): ", max_presence_purity_part)
    print("Prototype with part that has most often overlap with prototype: ", part_most_present)
...
def get_proto_patches_cub(net, projectloader, epoch, device, args, threshold=0.5):    
    ...
    with open(csvfilepath, "w", newline='') as csvfile:
        print("Collecting Prototype Image Patches for Evaluating CUB part purity. Writing CSV file with image patche coordinates..")
        writer = csv.writer(csvfile, delimiter=',')
        writer.writerow(columns)
        # Iterate through the prototypes and projection set
        img_iter = tqdm(enumerate(range(len(imgs))), total=len(imgs),ncols=0,desc='Collecting patch coordinates CUB')
        for _, imgid in img_iter:
            ...
            with torch.no_grad():
                ...
                pfs, _, (pooled, _), _, _ = net(imgtensor)
                ...
                for prototype in range(net.module._num_prototypes):
                    ...
                    if c_weight > 1e-5:#ignore prototypes that are not relevant to any class
                        if pooled[prototype].item()>threshold: #similarity score > threshold
                            ...
                            h_coor_min, h_coor_max, w_coor_min, w_coor_max = get_img_coordinates(args.image_size, args.wshape, patchsize, skip, location_h_idx[location_w_idx].item(), location_w_idx.item())
...
def get_topk_cub(net, projectloader, k, epoch, device, args):    
    ...
    too_small = set()
    protoype_iter = tqdm(enumerate(scores_per_prototype.keys()), total=len(list(scores_per_prototype.keys())),ncols=0,desc='Collecting top-k patch coordinates CUB')
    with open(csvfilepath, "w", newline='') as csvfile:
        print("Writing CSV file with top k image patches..")
        writer = csv.writer(csvfile, delimiter=',')
        ...
        for _, prototype in protoype_iter:
            ...
            for index, row in topk.iterrows():
                ...
                with torch.no_grad():
                    ...
                    h_coor_min, h_coor_max, w_coor_min, w_coor_max = get_img_coordinates(args.image_size, args.wshape, patchsize, skip, location[0], location[1])
                    proto_img_coordinates.append([prototype, imgname, h_coor_min, h_coor_max, w_coor_min, w_coor_max])
            # write intermediate results in case of large dataset
            if len(proto_img_coordinates)> 10000:
                writer.writerows(proto_img_coordinates) 
                proto_img_coordinates = []
        print("Warning: image patches included in topk, but similarity < 0.1! This might unfairly reduce the purity metric because prototype has less than k similar image patches. You could consider reducing k for prototypes", too_small)
