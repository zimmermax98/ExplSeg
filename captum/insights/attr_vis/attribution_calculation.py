...
class AttributionCalculation:
    ...
    def calculate_attribution(
        ...
        if attribution_method_name == "Layer Grad Cam":
            layer = [layer for layer in model.modules() if isinstance(layer, torch.nn.Conv2d)][-1]
            attribution_method = attribution_cls(model, layer)
        else:
        ...
        if attribution_method_name in ATTRIBUTION_METHOD_CONFIG:
            ...
            #        "layer": [module for module in model.modules() if not isinstance(module, torch.nn.Sequential)][-1]
            #    }
            #else:
            if True:
        ...
        if attribution_method_name == "Layer Grad Cam":
            attr = LayerAttribution.interpolate(attr, [data[0].shape[-2], data[0].shape[-1]], interpolate_mode="bilinear")
            attr = [attr]
