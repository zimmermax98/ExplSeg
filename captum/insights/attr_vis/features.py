...
class ImageFeature(BaseFeature):
    ...
    def visualize(self, attribution, data, contribution_frac, attribution_method) -> FeatureOutput:
        if self.visualization_transform:
            data = self.visualization_transform(data)

        data_t, attribution_t = [
            t.detach().squeeze(0).permute((1, 2, 0)).cpu().numpy()
        ...
        attr_fig, _ = viz.visualize_image_attr(
            attribution_t,
            data_t,
            method="blended_heat_map" if attribution_method == "Layer Grad Cam" else "heat_map",
            cmap="jet" if attribution_method == "Layer Grad Cam" else None,
            sign="absolute_value",
            use_pyplot=False,
        )
